...
.DESCRIPTION
    ...
    into packaging/qt/ via aqtinstall, then builds with CMake, gathers the
    Qt DLLs with windeployqt and zips a portable folder — no installer.
...
$qtRoot = Join-Path $PSScriptRoot "qt"
...
# --- 3. Deploy Qt runtime ----------------------------------------------------
$distDir = Join-Path $PSScriptRoot "dist\Masareef"
Write-Host "==> Deploying to $distDir" -ForegroundColor Cyan
if (Test-Path $distDir) { Remove-Item -Recurse -Force $distDir }
New-Item -ItemType Directory -Force -Path $distDir | Out-Null
Copy-Item (Join-Path $buildDir "masareef.exe") $distDir
...
& (Join-Path $qtPath "bin\windeployqt.exe") --release --compiler-runtime `
    --no-translations (Join-Path $distDir "masareef.exe")
